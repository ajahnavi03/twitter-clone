...
const MainPage = ({user}) => {
    ...
    useEffect(() => {
      fetch(`https://twitter-clone-1-7nsn.onrender.com/userPost?email=${user?.email}`)
      .then(res => res.json())
      .then(data => {
        setPosts(data)
      })
    ...
    const handleUploadCoverImage = (e) => {
        ...
        .then(res=> {
            ...
            setIsLoading(false);
            if(url){
                axios.patch(`https://twitter-clone-1-7nsn.onrender.com/userUpdates/${user?.email}`, userCoverImage)
            }
        })
    }
    const handleUploadProfileImage = (e) => {
        ...
        .then(res=> {
            ...
            setIsLoading(false);
            if(url){
                axios.patch(`https://twitter-clone-1-7nsn.onrender.com/userUpdates/${user?.email}`, userProfileImage)
            }
        })
    }
